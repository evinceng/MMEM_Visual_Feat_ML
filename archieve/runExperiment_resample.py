...
#10 is here just to try that the script work. Change it sth that makes sense
freq = 10.0#128.0 #30.0

resampleFigFolder = "/Figs_" + str(int(freq)) + "/"

resampleDataFolder = "/Resampled_" + str(int(freq)) + "/"
...
def resample(uID, file, df, tckDict, fileName, plotIndex):
    ...
    if np.min(xIn[1:] - xIn[:-1]) < 0:
        ...
        with open(rootFolder + str(uID) + resampleDataFolder + "decreasingTimes.txt", 'w') as f:
    ...
    for signalName in df.columns:
        ...
        plt.legend()
        # plt.show()
        plt.savefig(rootFolder + str(uID) + resampleFigFolder + title + str(plotIndex) + ".jpg")
        if 'Acc' in signalName or signalName == 'diameter' or signalName == 'EDA' or signalName == 'HR' or signalName == 'GSR_Skin_Conductance_microSiemens' or signalName == 'Temperature_BMP280_Degrees Celsius':
            pickle.dump(fig, open(rootFolder + str(uID) + resampleFigFolder + title + str(plotIndex) +'.pickle', 'wb')) # This is for Python 3 - py2 may need `file` instead of `open`
        plt.close(fig)
        plotIndex += 1
        
    resampled_df.to_csv(rootFolder + str(uID) + resampleDataFolder + fileName + "_resampled.csv")
    return plotIndex
...
def resampleuIDCsvFiles(rootFolder , uID):
    ...
    for file in glob.glob("*.csv"):
        ...
        pathlib.Path(rootFolder + str(uID) + resampleFigFolder).mkdir(parents=True, exist_ok=True)
        pathlib.Path(rootFolder + str(uID) + resampleDataFolder).mkdir(parents=True, exist_ok=True)
    ...
    with open(rootFolder + str(uID) + resampleDataFolder + "ExecutionTime.txt", 'w') as f:
